...
function createModelDetailsClient(pathTemplate, modelStructure, relativeDest) {
...
function createModelClient(pathTemplate, arrayModel, modelName, relativeDest) {
    const dirJoin = path.join(__dirname, pathTemplate)
    const read = fs.readFileSync(dirJoin, 'utf8')
...
function changeClientRoute(readContent, NameModel) {
    ...
    `import ${ NameModel[0].toUpperCase()+NameModel.slice(1,NameModel.length).toLowerCase() } from './${NameModel[0].toUpperCase()+NameModel.slice(1,NameModel.length).toLowerCase() }' \nimport ${ NameModel[0].toUpperCase()+NameModel.slice(1,NameModel.length).toLowerCase()}Detail from './${NameModel[0].toUpperCase()+NameModel.slice(1,NameModel.length).toLowerCase() }Details'
    `
