...
module.exports = async function bikinin(argv) {
    try {
        ...
        if (error.message === 'invalid type') {
            ...
            let str = chalk.yellow(`- Cara input model beserta attributenya:\n`) + chalk.blue(`\n  sulap bikinin --model [namaModel] --attributes [namaData]:[tipeData],[namaData]:[tipeData]\n`) + chalk.yellow(`\n- List tipe data yang bisa kamu gunakan:\n`) + chalk.blue(`\n  -string\n  -textarea\n  -image\n  -boolean\n  -number`)
            console.log(str)
        } else {
            spinner.fail()
            console.log(error)
